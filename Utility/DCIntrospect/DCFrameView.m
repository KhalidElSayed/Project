...
- (void)drawRect:(CGRect)rect {
    ...
    if (CGRectIsEmpty(self.mainRect)) return;
    ...
    if (!CGRectIsEmpty(self.superRect)) {
        if ((mainRectOffset.origin.x != floorf(mainRectOffset.origin.x) && mainRect.origin.x != 0) || (mainRectOffset.origin.y != floor(mainRectOffset.origin.y) && mainRect.origin.y != 0)) showAntialiasingWarning = YES;
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (frame.origin.x < 2.0f) frame.origin.x = 2.0f;
    else if (CGRectGetMaxX(frame) > self.bounds.size.width - 2.0f) frame.origin.x = self.bounds.size.width - frame.size.width - 2.0f;
    if (frame.origin.y < minY) frame.origin.y = touchPoint.y + stringSize.height + 4.0f;
