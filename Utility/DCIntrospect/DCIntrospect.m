...
+ (void)workaroundUITextInputTraitsPropertiesBug {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if (class_getInstanceMethod(class, NSSelectorFromString(@"textInputTraits"))) {
            ...
            if (!originalValueForKey) originalValueForKey = (IMP)[objc_getAssociatedObject([class superclass], originalValueForKeyIMPKey) pointerValue];
            if (!originalValueForKey) originalValueForKey = class_getMethodImplementation([class superclass], @selector(valueForKey:));
...
- (void)start {
    ...
    if (!self.currentViewHistory) self.currentViewHistory = [[[NSMutableArray alloc] init] autorelease];
...
- (void)takeFirstResponder {
    if (![self.inputTextView becomeFirstResponder]) NSLog(@"DCIntrospect: Couldn't reclaim keyboard input.  Is the keyboard used elsewhere?");
}
...
- (void)setKeyboardBindingsOn:(BOOL)areKeyboardBindingsOn {
    ...
    if (self.keyboardBindingsOn) [self.inputTextView becomeFirstResponder];
    else [self.inputTextView resignFirstResponder];
}
...
- (void)invokeIntrospector {
    ...
    if (self.on) {
        ...
        if (self.viewOutlines) [self toggleOutlines];
        if (self.highlightNonOpaqueViews) [self toggleNonOpaqueViews];
        if (self.showingHelp) [self toggleHelp];
...
- (void)selectView:(UIView *)view {
    ...
    if (![self.currentViewHistory containsObject:self.currentView]) [self.currentViewHistory addObject:self.currentView];
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView {
    if (!(self.on && self.handleArrowKeys)) return;
    ...
    if (shiftKey) {
        if (selectionLocation == 4 && selectionLength == 1) frame.origin.x -= 10.0f;
        else if (selectionLocation == 5 && selectionLength == 1) frame.origin.x += 10.0f;
        else if (selectionLocation == 0 && selectionLength == 5) frame.origin.y -= 10.0f;
        else if (selectionLocation == 5 && selectionLength == 5) frame.origin.y += 10.0f;
    } else if (optionKey) {
        if (selectionLocation == 7) frame.size.width += 1.0f;
        else if (selectionLocation == 3) frame.size.width -= 1.0f;
        else if (selectionLocation == 9) frame.size.height += 1.0f;
        else if (selectionLocation == 1) frame.size.height -= 1.0f;
    } else {
        if (selectionLocation == 4) frame.origin.x -= 1.0f;
        else if (selectionLocation == 6) frame.origin.x += 1.0f;
        else if (selectionLocation == 0) frame.origin.y -= 1.0f;
        else if (selectionLocation == 10) frame.origin.y += 1.0f;
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)string {
    ...
    if ([string isEqualToString:kDCIntrospectKeysToggleViewOutlines]) {
        ...
        [UIView animateWithDuration:0.15
                         ...
                         animations:^{
                             ...
                             if (self.showStatusBarOverlay) [self showTemporaryStringInStatusBar:coordinatesString];
                             else NSLog(@"DCIntrospect: %@", coordinatesString);
                         }];
        return NO;
    ...
    if (self.on && self.currentView) {
        if ([string isEqualToString:kDCIntrospectKeysLogProperties]) {
            ...
            if (self.currentViewHistory.count == 0) return NO;
        ...
        CGRect frame = self.currentView.frame;
        if ([string isEqualToString:kDCIntrospectKeysNudgeViewLeft]) frame.origin.x -= 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysNudgeViewRight]) frame.origin.x += 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysNudgeViewUp]) frame.origin.y -= 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysNudgeViewDown]) frame.origin.y += 1.0f;
            ...
            frame = CGRectMake(floorf((self.currentView.superview.frame.size.width - frame.size.width) / 2.0f),
                               ...
                               frame.size.width,
                               frame.size.height);
        else if ([string isEqualToString:kDCIntrospectKeysIncreaseWidth]) frame.size.width += 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysDecreaseWidth]) frame.size.width -= 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysIncreaseHeight]) frame.size.height += 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysDecreaseHeight]) frame.size.height -= 1.0f;
        else if ([string isEqualToString:kDCIntrospectKeysIncreaseViewAlpha]) {
            if (self.currentView.alpha < 1.0f) self.currentView.alpha += 0.05f;
        } else if ([string isEqualToString:kDCIntrospectKeysDecreaseViewAlpha]) {
            if (self.currentView.alpha > 0.0f) self.currentView.alpha -= 0.05f;
...
- (void)logCodeForCurrentViewChanges {
    ...
    if ([varName isEqualToString:[NSString stringWithFormat:@"%@", self.currentView.class]]) varName = @"<#view#>";
    ...
    if (outputString.length == 0) NSLog(@"DCIntrospect: No changes made to %@.", self.currentView.class);
    else printf("\n\n%s\n", [outputString UTF8String]);
}

- (void)setName:(NSString *)name forObject:(id)object accessedWithSelf:(BOOL)accessedWithSelf {
    if (!self.objectNames) self.objectNames = [NSMutableDictionary dictionary];

    if (accessedWithSelf) name = [@"self." stringByAppendingString : name];
...
- (NSString *)nameForObject:(id)object {
    ...
    if (!self.objectNames) return objectName;
...
- (void)removeNamesForViewsInView:(UIView *)view {
    if (!self.objectNames) return;

    NSMutableArray *objectsToRemove = [NSMutableArray array];
    [self.objectNames enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        if ([[obj class] isSubclassOfClass:[UIView class]]) {
            UIView *subview = (UIView *)obj;
            if ([self view:view containsSubview:subview]) [objectsToRemove addObject:key];
        }
    }];
...
- (void)updateFrameView {
    ...
    if (self.on) {
        if (self.currentView) {
            ...
            if (self.currentView.superview == mainWindow) self.frameView.superRect = CGRectZero;
            else if (self.currentView.superview.superview) self.frameView.superRect = [self.currentView.superview.superview convertRect:self.currentView.superview.frame toView:self.frameView];
            else self.frameView.superRect = CGRectZero;
        } else {
...
- (void)updateStatusBar {
    if (self.currentView) {
        ...
        if ([nameForObject hasPrefix:@"self."]) nameForObject = [nameForObject substringFromIndex:@"self.".length];

        if (self.currentView.tag != 0) self.statusBarOverlay.leftLabel.text = [NSString stringWithFormat:@"%@ (tag: %i)", nameForObject, self.currentView.tag];
        else self.statusBarOverlay.leftLabel.text = [NSString stringWithFormat:@"%@", nameForObject];
    ...
    if (self.showStatusBarOverlay) self.statusBarOverlay.hidden = NO;
    else self.statusBarOverlay.hidden = YES;
}
...
- (void)forceReloadOfView {
    if ([self.currentView class] == [UITableView class]) [(UITableView *)self.currentView reloadData];
}
...
- (void)toggleOutlines {
    ...
    if (self.viewOutlines) [self addOutlinesToFrameViewFromSubview:mainWindow];
    else [self.frameView.rectsToOutline removeAllObjects];

    [self.frameView setNeedsDisplay];

    NSString *string = [NSString stringWithFormat:@"Showing view outlines is %@", (self.viewOutlines) ? @"on":@"off"];
    if (self.showStatusBarOverlay) [self showTemporaryStringInStatusBar:string];
    else NSLog(@"DCIntrospect: %@", string);
}
...
- (void)setBackgroundColor:(UIColor *)color ofNonOpaqueViewsInSubview:(UIView *)view {
    for (UIView *subview in view.subviews) {
        ...
        if (!subview.opaque) subview.backgroundColor = color;
...
- (NSString *)describeProperty:(NSString *)propertyName value:(id)value {
    if ([propertyName isEqualToString:@"contentMode"]) {
        ...
        if (mask & UIViewAutoresizingFlexibleLeftMargin) [string appendString:@"UIViewAutoresizingFlexibleLeftMargin"];
        if (mask & UIViewAutoresizingFlexibleRightMargin) [string appendString:@" | UIViewAutoresizingFlexibleRightMargin"];
        if (mask & UIViewAutoresizingFlexibleTopMargin) [string appendString:@" | UIViewAutoresizingFlexibleTopMargin"];
        if (mask & UIViewAutoresizingFlexibleBottomMargin) [string appendString:@" | UIViewAutoresizingFlexibleBottomMargin"];
        if (mask & UIViewAutoresizingFlexibleWidth) [string appendString:@" | UIViewAutoresizingFlexibleWidthMargin"];
        if (mask & UIViewAutoresizingFlexibleHeight) [string appendString:@" | UIViewAutoresizingFlexibleHeightMargin"];

        if ([string hasPrefix:@" | "]) [string replaceCharactersInRange:NSMakeRange(0, 3) withString:@""];
        ...
        NSMutableString *string = [NSMutableString string];
        if (traits & UIAccessibilityTraitButton) [string appendString:@"UIAccessibilityTraitButton"];
        if (traits & UIAccessibilityTraitLink) [string appendString:@" | UIAccessibilityTraitLink"];
        if (traits & UIAccessibilityTraitSearchField) [string appendString:@" | UIAccessibilityTraitSearchField"];
        if (traits & UIAccessibilityTraitImage) [string appendString:@" | UIAccessibilityTraitImage"];
        if (traits & UIAccessibilityTraitSelected) [string appendString:@" | UIAccessibilityTraitSelected"];
        if (traits & UIAccessibilityTraitPlaysSound) [string appendString:@" | UIAccessibilityTraitPlaysSound"];
        if (traits & UIAccessibilityTraitKeyboardKey) [string appendString:@" | UIAccessibilityTraitKeyboardKey"];
        if (traits & UIAccessibilityTraitStaticText) [string appendString:@" | UIAccessibilityTraitStaticText"];
        if (traits & UIAccessibilityTraitSummaryElement) [string appendString:@" | UIAccessibilityTraitSummaryElement"];
        if (traits & UIAccessibilityTraitNotEnabled) [string appendString:@" | UIAccessibilityTraitNotEnabled"];
        if (traits & UIAccessibilityTraitUpdatesFrequently) [string appendString:@" | UIAccessibilityTraitUpdatesFrequently"];
        if (traits & UIAccessibilityTraitStartsMediaSession) [string appendString:@" | UIAccessibilityTraitStartsMediaSession"];
        if (traits & UIAccessibilityTraitAdjustable) [string appendFormat:@" | UIAccessibilityTraitAdjustable"];
        if ([string hasPrefix:@" | "]) [string replaceCharactersInRange:NSMakeRange(0, 3) withString:@""];
...
- (void)toggleHelp {
    ...
    if (self.showingHelp) {
        ...
        if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) [helpString appendString:@"body { font-size:11pt; width:500px; margin:0 auto; }"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([requestString isEqualToString:@"about:blank"]) return YES;
    else if ([requestString isEqualToString:@"http://close/"]) [self toggleHelp];
            ...
            for (unsigned int i = 0; i < count; ++i) {
                ...
                if ([object respondsToSelector:sel]) {
                    ...
                    @try {
                        ...
                        if ([returnType isEqualToString:@"c"]) returnObject = [NSNumber numberWithBool:[returnObject intValue] != 0];
...
- (BOOL)view:(UIView *)view containsSubview:(UIView *)subview {
    for (UIView *aView in view.subviews) {
        if (aView == subview) return YES;

        if ([self view:aView containsSubview:subview]) return YES;
    }
...
- (BOOL)shouldIgnoreView:(UIView *)view {
    if (view == self.frameView || view == self.inputTextView) return YES;
    return NO;
}
