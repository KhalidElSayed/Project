...
- (void)setAutoAdjustTopOffset:(BOOL)autoAdjustTopOffset {
    if (_autoAdjustTopOffset != autoAdjustTopOffset) {
        ...
        if (autoAdjustTopOffset) autoresizingMask |= UIViewAutoresizingFlexibleTopMargin;
        else autoresizingMask ^= UIViewAutoresizingFlexibleTopMargin;
...
- (CGFloat)previousTextHeight {
    if (!_previousTextHeight) _previousTextHeight = [self bounds].size.height;
...
- (void)didPressButton {
    if ([[self delegate] respondsToSelector:@selector(composeBarViewDidPressButton:)]) [[self delegate] composeBarViewDidPressButton:self];
}

- (void)didPressUtilityButton {
    if ([[self delegate] respondsToSelector:@selector(composeBarViewDidPressUtilityButton:)]) [[self delegate] composeBarViewDidPressUtilityButton:self];
}
...
- (void)resizeTextViewIfNeededAnimated:(BOOL)animated {
    ...
    if (textHeightDelta == 0.0f && [self bounds].size.height == maxViewHeight) return;
    ...
    if (yOffset != [[self textView] contentOffset].y) scroll = ^{ [(PHFComposeBarView_TextView *)[self textView] PHFComposeBarView_setContentOffset : CGPointMake(0.0f, yOffset)]; };
    ...
    if (viewHeightDelta) {
        ...
        if ([self autoAdjustTopOffset]) frameEnd.origin.y -= viewHeightDelta;
...
- (void)scrollToCaretIfNeeded {
    ...
    if ([selectedTextRange isEmpty]) {
        ...
        // Caret is in or below visible part of text view:
            ...
            if (relativeCaretY > maxY) offsetYDelta = relativeCaretY - maxY;
        }
...
- (CGFloat)textHeight {
    ...
    // needed.
    if ([[[self textView] text] isEqualToString:@""]) return PHFComposeBarViewInitialHeight;
    else return [[self textView] contentSize].height + kTextViewToSuperviewHeightDelta;
}
