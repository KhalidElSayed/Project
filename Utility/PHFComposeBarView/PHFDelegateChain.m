...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)selector {
    ...
    if (!object) [NSException raise:NSInvalidArgumentException format:@"%@ was unable to handle %@ because none of the objects %.@ responds to it.", self, NSStringFromSelector(selector), _objects];
...
- (BOOL)conformsToProtocol:(Protocol *)protocol {
    for (id object in _objects) {
        if ([object conformsToProtocol:protocol]) return YES;
    }
    return NO;
}
...
- (id)__firstObjectRespondingToSelector:(SEL)selector {
    ...
    if (index != NSNotFound) object = [_objects objectAtIndex:index];
